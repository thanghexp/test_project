...
(function () {
    ...
    /**
     * TEXIS.History Class for all pages
     *
     * @type {void|*}
     */
    TEXIS.History = AppCore.Base.extend({
                ...
                success: function (res) {
                    ...
                    if (res.success == true && res.submit == true) {
                        ...
                        _.each(data, function (item) {
                            ...
                            // Build html show status
                            if (item.extra_data.status_name) {
                                html_status = '<li class="time-label x-log-status"><span class="bg-green">'+ item.extra_data.status_name +'</span></li>';
                            }

                            // Assign data
                            $(new_timeline).find('.x-log-time').html('<i class="fa fa-clock-o margin-r-5"></i>' + item.create_at);
                            $(new_timeline).find('.x-log-header').html('<strong>' + item.account_name + '</strong>');
                            $(new_timeline).find('.x-log-body').html(item.message);

                            // Append at last list
                            last_item.after(html_status, new_timeline);
                        });
                    }
                },
                error: function (res) {
