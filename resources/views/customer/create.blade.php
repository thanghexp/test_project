	...
	<div class="col-md-12">
		<div class="box box-primary">
			<div class="box-header with-border">
				<h3 class="box-title"><i class="fa fa-<!--{if !empty($edit_data)}-->edit<!--{else}-->plus-circle<!--{/if}--> margin-r-5 text-primary"></i><!--{$page_title}--></h3>
			</div>
						...
						<div class="col-md-6">
							...
							<div class="form-group">
								...
								<select id="status" class="form-control select2" name="status">
									<!--{foreach from=$customer_status item=status}-->
									<option value="<!--{$status.code|escape}-->" <!--{if isset($data_customer.status) && $data_customer.status == $status.code}-->selected<!--{/if}-->><!--{$status.value|escape|default:''}--></option>
									<!--{/foreach}-->
								</select>
							</div>
							<div class="form-group">
				...
				<div class="box-footer">
					<div class="row">
						<div class="col-md-4 col-sm-6 col-xs-6 col-md-offset-2">
							<a id="x-button-cancel" href="/customer/<!--{$data_customer.id|default:''}-->" class="btn btn-block btn-default"><b><i class="fa fa-ban margin-r-5"></i>キャンセル</b></a>
						</div>
